...
type domainCheck func(context.Context, string, rtypes.ResourceRecordSet)

func checkError(err error, t, k, zone string, name string) {
	var herr *HTTPError
	if errors.As(err, &herr) {
		switch herr.Reason {
		case "SSL not configured":
			log.Printf("%s Zone %s has %s %s to %s but SSL is not configured\n", MISCONFIG, zone, t, k, name)
...
func checkAliasCloudFront(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
	if record.AliasTarget != nil && strings.HasSuffix(aws.ToString(record.AliasTarget.DNSName), ".cloudfront.net") && record.Type != rtypes.RRTypeAaaa {
		...
		if err != nil {
			checkError(err, "an alias", "CloudFront", zone, name)
		}
		if nok {
...
func checkCnameCloudFront(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
	if record.Type == rtypes.RRTypeCname && len(record.ResourceRecords) >= 1 && strings.HasSuffix(aws.ToString(record.ResourceRecords[0].Value), ".cloudfront.net") {
		...
		nok, err := checkNoSuchBucket(ctx, name)
		if err != nil {
			checkError(err, "a CNAME", "CloudFront", zone, name)
		}
		if nok {
...
func checkAliasElasticBeanStalk(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
...
func checkCnameElasticBeanStalk(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
...
func checkCnameS3(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
	if record.Type == rtypes.RRTypeCname &&
		...
		if err != nil {
			checkError(err, "a CNAME", "S3", zone, name)
		}
		if nok {
...
func checkAliasS3(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
	if record.AliasTarget != nil &&
		...
		nok, err := checkNoSuchBucket(ctx, dst)
		if err != nil {
			checkError(err, "an alias", "S3", zone, name)
		}
		if nok {
...
func SubDomainTakeoverCheck(ctx context.Context, zone string, record rtypes.ResourceRecordSet) {
	for _, check := range subDomainTakeoverChecks {
		check(ctx, zone, record)
	}
}
